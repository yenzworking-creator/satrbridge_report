...
class DatabaseManager:
    ...
    def load_data_lazily(self):
        ...
        self.is_loaded = False
        self.village_data = {}
        self.mrt_data = {} # New: MRT Flow Data
        self.cache_file = os.path.join(self.data_dir, "db_cache.pkl")
        
        # Try load from cache
        if os.path.exists(self.cache_file):
            print("⚡ 發現快取檔案，正在快速載入資料庫...")
            try:
                with open(self.cache_file, 'rb') as f: # Changed cache_file to self.cache_file
                    data = pickle.load(f)
                    self.pop_df = data['pop']
                    self.tax_df = data['tax']
                    self.mrt_df = data['mrt']
                    self.rent_df = data['rent']
                # Load Extra MRT Data (assuming this is a new method to be called after loading mrt_df)
                # Note: The original instruction snippet had `self.data = df` and `self.build_village_index()`
                # which are not present in the original code and seem like unrelated edits.
                # I'm only adding `self.load_mrt_data()` as per the explicit instruction.
                # If `self.load_mrt_data()` depends on `self.data` or `build_village_index`,
                # those would need to be defined elsewhere or added.
                # For now, I'm assuming `self.load_mrt_data()` is a standalone call.
                self.load_mrt_data()
    ...
    def get_rental_analysis(self, city, district, address_road):
        ...
        return stats
    # --- MRT Data Integration (Oct 2025) ---
    def load_mrt_data(self):
        """
        Load MRT Daily Flow from Excel
        """
        try:
            mrt_path = os.path.join(self.data_dir, "桃園捷運各站運量日統計表_202510.xlsx")
            if not os.path.exists(mrt_path):
                print(f"⚠️  MRT file not found: {mrt_path}")
                return

            print(f"🚅  載入捷運運量資料: {mrt_path}")
            df = pd.read_excel(mrt_path)
            
            # Expected Columns based on user image: [年度, 月份, 捷運站別, 平均日運量]
            # Name format: "A1 台北車站"
            # We urge robust parsing
            
            for index, row in df.iterrows():
                try:
                    full_name = str(row.get('捷運站別', '')).strip() # e.g. "A18 高鐵桃園站"
                    flow = row.get('平均日運量', 0)
                    
                    if not full_name: continue
                    
                    # Store Full Name
                    self.mrt_data[full_name] = flow
                    
                    # Split Code and Name (e.g. "A18" and "高鐵桃園站")
                    parts = full_name.split(' ')
                    if len(parts) >= 2:
                        code = parts[0].strip() # A18
                        name = parts[1].strip() # 高鐵桃園站
                        
                        self.mrt_data[code] = flow
                        self.mrt_data[name] = flow
                        
                        # Handle potential space inconsistency "A18高鐵桃園站"
                        merged = code + name
                        self.mrt_data[merged] = flow
                except Exception as row_e:
                    print(f"Error parsing MRT row {index}: {row_e}")

            print(f"✅ 捷運資料載入完成，共索引 {len(self.mrt_data)} 筆關鍵字")

        except Exception as e:
            print(f"❌ MRT Data Load Failed: {e}")

    def get_mrt_flow(self, station_name):
        """
        Fuzzy lookup for MRT Flow
        """
        if not station_name: return 0
        
        # 1. Direct Lookup
        if station_name in self.mrt_data:
            return self.mrt_data[station_name]
            
        # 2. Fuzzy / Partial Match
        # If input is "捷運高鐵桃園站" -> try finding "高鐵桃園站" in keys
        for key in self.mrt_data:
            if key in station_name or station_name in key:
                # Avoid short matches like "A1" matching "A18"
                if len(key) > 2 and len(station_name) > 2:
                     return self.mrt_data[key]
