...
window.renderDashboard = function (data) {
    ...
    // --- PDF Header & Footer Injection (Visual Only) ---
    // DISABLED: User requested "Exactly like Web" and duplicate fixes.
    // We no longer inject these extra elements.
    ...
    // Remove functionality to prevent duplicates and layout shift
    ...
    */
    ...
    // DEBUG: Log keys to help troubleshoot
    console.log("Map Debug:", {
        hasImg: !!mapImg,
        lat: aiResult.lat,
        lng: aiResult.lng,
        hasKey: !!aiResult.google_maps_key,
        keyLen: aiResult.google_maps_key ? aiResult.google_maps_key.length : 0
    });

    // Check if lat/lng are defined (allowing 0)
    const hasCoords = aiResult.lat !== undefined && aiResult.lng !== undefined;

    // Check for Placeholder Key
    const PLACEHOLDER_KEY = "AIzaSyD9oKQX7ZGL17WsW6R3-vuXV0VCQybWdhY";
    const isDefaultKey = aiResult.google_maps_key === PLACEHOLDER_KEY;

    if (mapImg && hasCoords && aiResult.google_maps_key && !isDefaultKey) {
        ...
        mapImg.classList.remove('hidden'); // Ensure class doesn't hide it
        mapImg.src = mapUrl;
        mapImg.style.display = 'block';

        if (mapPlace) mapPlace.style.display = 'none';

        // Handle Error using explicit onerror handler injection
        mapImg.onerror = function () {
            console.error("Google Map Load Failed (403/404)");
            this.style.display = 'none';
            if (mapPlace) {
                mapPlace.style.display = 'block';
                mapPlace.innerHTML = '<div style="color:#e74c3c; font-weight:bold;"><i class="fa-solid fa-triangle-exclamation"></i> 地圖載入失敗</div><div style="font-size:0.85rem; margin-top:5px; color:#666;">可能原因：API Key 未啟用或網域限制</div>';
            }
        };
        ...
        if (mapPlace) {
            mapPlace.style.display = 'block';
            if (isDefaultKey) {
                mapPlace.innerHTML = '<div style="color:#e67e22; font-weight:bold;"><i class="fa-solid fa-key"></i> 尚未設定 API Key</div><div style="font-size:0.85rem; margin-top:5px; color:#666;">請在後台 (config.py) 更新 Google Maps 金鑰</div>';
            } else {
                mapPlace.textContent = '無地圖資料 (經緯度遺失)';
            }
        }
    }
    ...
    const setMetric = (id, val, prefix = 'NT$ ') => {
        ...
        // Explicitly check against null/undefined/empty string, allowing 0
        const isValid = val !== null && val !== undefined && val !== '';
        if (el) el.textContent = isValid ? prefix + formatNumber(val) : '-';
    };

    setMetric('val-revenue', (aiResult.daily_revenue || 0) * 30);
    setMetric('val-turnover', aiResult.turnover_rate, '');

    // Support aliases for ROI and Traffic
    const roiVal = aiResult.return_period_months || aiResult.roi_period || aiResult.roi;
    setMetric('val-roi', roiVal, '');

    const trafficVal = aiResult.est_daily_traffic || aiResult.daily_traffic || aiResult.traffic;
    setMetric('val-traffic', trafficVal, '');
...
function initCharts(data) {
    ...
    const commonOptions = {
        ...
        layout: { padding: { top: 20 } }, // Add padding for labels
        plugins: {
            legend: { position: 'bottom', labels: { color: '#666', font: { size: 12 } } },
            datalabels: {
                display: 'auto', // Smart display to avoid overlap
                color: '#444',
                ...
                offset: 4,
                formatter: (value) => {
                    // Smart formatting: add percent for small fractions, or commas for big numbers
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        datalabels: {
                            ...
                            formatter: (value, ctx) => {
                                // Add Line Break for label if possible, or just value + %
                                return value + '%';
                            },
                            anchor: 'center',
                            align: 'center'
...
// Helper: Format text (convert newlines to <br> and bold headers)
function formatText(text) {
    if (!text) return '';

    // 1. Process bullet points like (1), 1., or 【Title】
    // Bold the starting key part: "(1) Title :" -> "<b>(1) Title :</b>"
    ...
    let formatted = text.split('\n').map(function (line) {
        ...
        if (line.startsWith('【') && line.includes('】')) {
            line = line.replace(/^(【.*?】)/, '<strong style="color:#2c3e50; font-size:1.15rem; display:block; margin-bottom:6px;">$1</strong>');
            return '<div style="margin-top:15px; margin-bottom:10px;">' + line + '</div>';
        }

        // Pattern B: (1), (2) or 1., 2.
        // Enhanced: Add slight indentation and better spacing
        const listRegex = /^(\(?\d+[\).\s]|●|•|-)\s*(.*?)(\:|：)/;

        if (listRegex.test(line)) {
            line = line.replace(listRegex, '<strong style="color:#d35400; font-weight:700;">$1 $2$3</strong>');
            return '<p style="margin-bottom:12px; line-height:1.8; font-size:1.05rem; color:#34495e;">' + line + '</p>';
        }
